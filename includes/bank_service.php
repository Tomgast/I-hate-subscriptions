...
class BankService {
     ...
     * Creates scan record and returns TrueLayer authorization URL
     * @param int $userId User ID
     * @param string $planType Plan type for tracking
     * @return string|false Authorization URL or false on error
     */
    public function initiateBankConnection($userId, $planType = 'monthly') {
        try {
            // Create scan record
            $scanId = $this->createScanRecord($userId, $planType);
            if (!$scanId) {
                throw new Exception("Failed to create scan record");
            }
            
            // Generate TrueLayer authorization URL
            $authUrl = $this->getBankAuthorizationUrl($userId);
            if (!$authUrl) {
            ...
            error_log("Error initiating bank connection: " . $e->getMessage());
            return false;
        }
    }
     ...
     * @param int $userId User ID
     * @param string $planType Plan type
     * @return int|false Scan ID or false on error
     */
    private function createScanRecord($userId, $planType) {
        try {
            // Create bank_scans table if it doesn't exist
            $this->pdo->exec("
                CREATE TABLE IF NOT EXISTS bank_scans (
                    ...
                    subscriptions_found INT DEFAULT 0,
                    total_monthly_cost DECIMAL(10,2) DEFAULT 0.00,
                    scan_data TEXT NULL,
                    error_message TEXT NULL,
            ...
            // Insert scan record
            $stmt = $this->pdo->prepare("
                INSERT INTO bank_scans (user_id, plan_type, status, created_at)
