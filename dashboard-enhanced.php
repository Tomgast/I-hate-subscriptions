...
// ENHANCED ANALYTICS - Add AI-powered insights without breaking existing functionality
$potentialSubscriptions = [];
$categoryInsights = [];
try {
    $pdo = getDBConnection();
    
    // Get potential subscriptions from transaction data
    $stmt = $pdo->prepare("
        SELECT 
            merchant_name,
    ...
    ");
    $stmt->execute([$userId]);
    $rawPotentials = $stmt->fetchAll();
    
    // Filter out existing subscriptions
    foreach ($rawPotentials as $potential) {
        $isKnown = false;
        foreach ($subscriptions as $sub) {
            if (stripos($sub['name'], $potential['merchant_name']) !== false) {
                $isKnown = true;
                break;
            }
        }
    ...
    // Ignore errors - enhanced features are optional
    error_log("Enhanced analytics error: " . $e->getMessage());
}
